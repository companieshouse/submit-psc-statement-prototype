...
router.post('/v5/confirm-company', function (req, res) {
  if ((req.session.data['companyNumber'] == '22223333')) {
    // Super secure
    res.redirect('/v5/super-secure')
  } else if ((req.session.data['companyNumber'] == '66667777')){
    // No PSCs or RLEs
    res.redirect('/v5/no-pscs')
  } else {
    res.redirect('/v5/psc-type')
  }
})


// ******* psc-type javascript ********************************
router.get('/v5/psc-type', function (req, res) {
  // Set URl
  res.render('v5/psc-type', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/psc-type', function (req, res) {
  var errors = []
  ...
  if (typeof req.session.data['pscType'] === 'undefined') {
    ...
    if ((req.session.data['pscType'] == 'rle')) {
      res.redirect('/v5/rle/rle-list')
    } else {
      res.redirect('/v5/individual/psc-list')
    }
  }
})
...
// ******* rle-list javascript ********************************
router.get('/v5/rle/rle-list', function (req, res) {
  res.render('v5/rle/rle-list', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/rle/rle-list', function (req, res) {
  var errors = []
  ...
  if (typeof req.session.data['rleList'] === 'undefined') {
    ...
    res.render('v5/rle/rle-list', {
      ...
      res.redirect('/v5/rle/ro-details')
    }
})

// ******* ro-details javascript *********************
router.get('/v5/rle/ro-details', function (req, res) {
  // Set URl
  res.render('v5/rle/ro-details', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/rle/ro-details', function (req, res) {
  // Create empty array and set error variables to false
  var errors = [];
  ...
  res.render('v5/rle/ro-details', {
    ...
    if (req.session.data['roPersonalCode'] === '111-2222-3333') {
      res.redirect('/v5/rle/ro-why-this-name')
    } 
    // dob code mis-match
      ...
      res.redirect('/v5/too-many-attempts')
    } // Director too young
      ...
      res.redirect('/v5/rle/ro-director')
    }
  }
})
...
router.get('/v5/rle/ro-director', function (req, res) {
  // Set URl
  res.render('v5/rle/ro-director', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/rle/ro-director', function (req, res) {
  // Create empty array and set error variables to false
  var errors = [];
  ...
  if (typeof req.session.data['roDirector'] === 'undefined') {
    ...
    if (req.session.data['roDirector'] === 'yes') {
      res.redirect('/v5/rle/ro-statements')
    } else {
      // User inputted value so move to next page
      res.redirect('/v5/rle/not-director-stop')
    }
  }
})
...
router.get('/v5/rle/ro-why-this-name', function (req, res) {
  // Set URl
  res.render('v5/rle/ro-why-this-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/rle/ro-why-this-name', function (req, res) {
  res.redirect('/v5/rle/ro-director')
})


// ******* ro-statements javascript ********************************
router.get('/v5/rle/ro-statements', function (req, res) {
  // Set URl
  res.render('v5/rle/ro-statements', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/rle/ro-statements', function (req, res) {
  res.redirect('/v5/rle/rle-verified')
})
...
// ******* psc-list javascript ********************************
router.get('/v5/individual/psc-list', function (req, res) {
  res.render('v5/individual/psc-list', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/individual/psc-list', function (req, res) {
  var errors = []
  ...
  if (typeof req.session.data['pscList'] === 'undefined') {
    ...
    res.render('v5/individual/psc-list', {
      ...
      res.redirect('/v5/individual/psc-details')
    }
})

// ******* psc-details javascript *********************
router.get('/v5/individual/psc-details', function (req, res) {
  // Set URl
  res.render('v5/individual/psc-details', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/individual/psc-details', function (req, res) {
  // Create empty array and set error variables to false
  var errors = [];
  ...
  res.render('v5/individual/psc-details', {
    ...
    if (req.session.data['pscPersonalCode'] === '111-2222-3333') {
      res.redirect('/v5/individual/psc-why-this-name')
    } 
    // dob code mis-match
      ...
      res.redirect('/v5/individual/psc-statement')
    }
  }
})
...
router.get('/v5/individual/personal-code', function (req, res) {
  // Set URl
  res.render('v5/individual/personal-code', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/individual/personal-code', function (req, res) {
// Create empty array and set error variables to false
var errors = []
...
if (req.session.data['personalCode'] === '') {
  ...
  res.render('v5/individual/personal-code', {
    ...
    if (req.session.data['personalCode'] === '111-2222-3333') {
      ...
      res.redirect('/v5/individual/non-match')
...
router.get('/v5/individual/psc-why-this-name', function (req, res) {
  // Set URl
  res.render('v5/individual/psc-why-this-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/individual/psc-why-this-name', function (req, res) {
  res.redirect('/v5/individual/psc-statement')
})
...
router.get('/v5/individual/psc-statement', function (req, res) {
  // Set URl
  res.render('v5/individual/psc-statement', {
    currentUrl: req.originalUrl
  })
})

router.post('/v5/individual/psc-statement', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['individualStatement'] === 'undefined') {
    ...
    res.render('v5/individual/psc-statement', {
      ...
      res.redirect('/v5/individual/psc-verified')
  }
})
