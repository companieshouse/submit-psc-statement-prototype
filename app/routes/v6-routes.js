...
router.get('/v6/company-number', function (req, res) {
  // Set URl
  res.render('v6/company-number', {
    currentUrl: req.originalUrl
  })
})
...
router.post('/v6/company-number', function (req, res) {
  ...
  if (req.session.data['companyNumber'] === '') {
    ...
    res.render('v6/company-number', {
      ...
      res.redirect('/v6/confirm-company')
  }
})


// ******* confirm-company javascript **********************
router.get('/v6/confirm-company', function (req, res) {
  // Set URl
  res.render('v6/confirm-company', {
    currentUrl: req.originalUrl
  })
})
...
router.post('/v6/confirm-company', function (req, res) {
  if ((req.session.data['companyNumber'] == '22223333')) {
    // Super secure
    res.redirect('/v6/super-secure')
  } else if ((req.session.data['companyNumber'] == '66667777')){
    // No PSCs or RLEs
    res.redirect('/v6/no-pscs')
  } else {
    res.redirect('/v6/psc-type')
  }
})


// ******* psc-type javascript ********************************
router.get('/v6/psc-type', function (req, res) {
  // Set URl
  res.render('v6/psc-type', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/psc-type', function (req, res) {
  var errors = []
  ...
  if (typeof req.session.data['pscType'] === 'undefined') {
    ...
    if ((req.session.data['pscType'] == 'rle')) {
      res.redirect('/v6/rle/rle-list')
    } else {
      res.redirect('/v6/individual/psc-list')
    }
  }
})
...
// ******* rle-list javascript ********************************
router.get('/v6/rle/rle-list', function (req, res) {
  res.render('v6/rle/rle-list', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/rle/rle-list', function (req, res) {
  var errors = []
  ...
  if (typeof req.session.data['rleList'] === 'undefined') {
    ...
    res.render('v6/rle/rle-list', {
      ...
      res.redirect('/v6/rle/ro-details')
    }
})

// ******* ro-details javascript *********************
router.get('/v6/rle/ro-details', function (req, res) {
  // Set URl
  res.render('v6/rle/ro-details', {
    currentUrl: req.originalUrl
  })
})
...
router.post('/v6/rle/ro-details', function (req, res) {
  ...
  res.render('v6/rle/ro-details', {
    ...
    if (req.session.data['roPersonalCode'] === '111-2222-3333') {
      res.redirect('/v6/rle/ro-why-this-name')
    } 
    // dob code mis-match
      ...
      res.redirect('/v6/too-many-attempts')
    } // Director too young
      ...
      res.redirect('/v6/rle/ro-director')
    }
  }
})
...
router.get('/v6/rle/ro-director', function (req, res) {
  // Set URl
  res.render('v6/rle/ro-director', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/rle/ro-director', function (req, res) {
  // Create empty array and set error variables to false
  var errors = [];
  ...
  if (typeof req.session.data['roDirector'] === 'undefined') {
    ...
    if (req.session.data['roDirector'] === 'yes') {
      res.redirect('/v6/rle/ro-statements')
    } else {
      // User inputted value so move to next page
      res.redirect('/v6/rle/not-director-stop')
    }
  }
})
...
router.get('/v6/rle/ro-why-this-name', function (req, res) {
  // Set URl
  res.render('v6/rle/ro-why-this-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/rle/ro-why-this-name', function (req, res) {
  res.redirect('/v6/rle/ro-director')
})


// ******* ro-statements javascript ********************************
router.get('/v6/rle/ro-statements', function (req, res) {
  // Set URl
  res.render('v6/rle/ro-statements', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/rle/ro-statements', function (req, res) {
  res.redirect('/v6/rle/rle-verified')
})
...
// ******* psc-list javascript ********************************
router.get('/v6/individual/psc-list', function (req, res) {
  res.render('v6/individual/psc-list', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/individual/psc-list', function (req, res) {
  var errors = []
  ...
  if (typeof req.session.data['pscList'] === 'undefined') {
    ...
    res.render('v6/individual/psc-list', {
      ...
      res.redirect('/v6/individual/psc-details')
    }
})

// ******* psc-details javascript *********************
router.get('/v6/individual/psc-details', function (req, res) {
  // Set URl
  res.render('v6/individual/psc-details', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/individual/psc-details', function (req, res) {
  // Create empty array and set error variables to false
  var errors = [];
  ...
  res.render('v6/individual/psc-details', {
    ...
    if (req.session.data['pscPersonalCode'] === '111-2222-3333') {
      res.redirect('/v6/individual/psc-why-this-name')
    } 
    // dob code mis-match
    else if (req.session.data['pscPersonalCode'] === '444-5555-6666') {
      res.redirect('/v6/individual/psc-dob-mismatch')
    } 
    else {
      res.redirect('/v6/individual/psc-statement')
    }
  }
})
...
router.get('/v6/individual/personal-code', function (req, res) {
  // Set URl
  res.render('v6/individual/personal-code', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/individual/personal-code', function (req, res) {
// Create empty array and set error variables to false
var errors = []
...
if (req.session.data['personalCode'] === '') {
  ...
  res.render('v6/individual/personal-code', {
    ...
    if (req.session.data['personalCode'] === '111-2222-3333') {
      ...
      res.redirect('/v6/individual/non-match')
...
router.get('/v6/individual/psc-why-this-name', function (req, res) {
  // Set URl
  res.render('v6/individual/psc-why-this-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/individual/psc-why-this-name', function (req, res) {
  res.redirect('/v6/individual/psc-statement')
})
...
router.get('/v6/individual/psc-statement', function (req, res) {
  // Set URl
  res.render('v6/individual/psc-statement', {
    currentUrl: req.originalUrl
  })
})

router.post('/v6/individual/psc-statement', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['individualStatement'] === 'undefined') {
    ...
    res.render('v6/individual/psc-statement', {
      ...
      res.redirect('/v6/individual/psc-verified')
  }
})
