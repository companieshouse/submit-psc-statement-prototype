...
router.get('/v11/sign-in-email', function (req, res) {
  // Set URl
  res.render('/v11/sign-in-email', {
    currentUrl: req.originalUrl
  })
})

router.post('/v11/sign-in-email', function (req, res) {
...
if (req.session.data['signin-email'] === '') {
  ...
  res.redirect('/v11/sign-in-password')
}
})


// ******* Sign in password validation ********************************
router.get('/v11/sign-in-password', function (req, res) {
// Set URl
res.render('/v11/sign-in-password', {
  currentUrl: req.originalUrl
})
})

router.post('/v11/sign-in-password', function (req, res) {
// Create empty array and set error variables to false
var errors = []
...
router.get('/v11/company-number', function (req, res) {
  // Set URl
  res.render('/v11/company-number', {
    currentUrl: req.originalUrl
  })
})
...
router.post('/v11/company-number', function (req, res) {
  ...
  if (req.session.data['companyNumber'] === '') {
    ...
    res.render('/v11/company-number', {
      ...
      res.redirect('/v11/confirm-company')
  }
})


// ******* confirm-company javascript **********************
router.get('/v11/confirm-company', function (req, res) {
  // Set URl
  res.render('/v11/confirm-company', {
    currentUrl: req.originalUrl
  })
})
...
router.get('/v11/individual/psc-details', function (req, res) {
  // Set URl
  res.render('/v11/individual/psc-details', {
    currentUrl: req.originalUrl
  })
})

router.post('/v11/individual/psc-details', function (req, res) {
  // Create empty array and set error variables to false
  var errors = [];
  ...
  res.render('/v11/individual/psc-details', {
    ...
    if (req.session.data['pscPersonalCode'] === '111-2222-3333') {
      res.redirect('/v11/individual/psc-why-this-name')
    } 
    // dob code mis-match
    else if (req.session.data['pscPersonalCode'] === '444-5555-6666') {
      res.redirect('/v11/individual/psc-dob-mismatch')
    } 
    else {
      res.redirect('/v11/individual/psc-statement')
    }
  }
})
...
router.get('/v11/individual/personal-code', function (req, res) {
  // Set URl
  res.render('/v11/individual/personal-code', {
    currentUrl: req.originalUrl
  })
})

router.post('/v11/individual/personal-code', function (req, res) {
// Create empty array and set error variables to false
var errors = []
...
if (req.session.data['personalCode'] === '') {
  ...
  res.render('/v11/individual/personal-code', {
    ...
    if (req.session.data['personalCode'] === '111-2222-3333') {
      ...
      res.redirect('/v11/individual/non-match')
    } else if (req.session.data['personalCode'] === 'aaa-bbbb-cccc') {
      res.redirect('/v11/too-many-attempts')
    } else {
      res.redirect('/v11/individual/psc-statement')
    }
  }
...
router.get('/v11/individual/psc-why-this-name', function (req, res) {
  // Set URl
  res.render('/v11/individual/psc-why-this-name', {
    currentUrl: req.originalUrl
  })
})

router.post('/v11/individual/psc-why-this-name', function (req, res) {
  res.redirect('/v11/individual/psc-statement')
})
...
router.get('/v11/individual/psc-statement', function (req, res) {
  // Set URl
  res.render('/v11/individual/psc-statement', {
    currentUrl: req.originalUrl
  })
})

router.post('/v11/individual/psc-statement', function (req, res) {
  // Create empty array
  var errors = []
  ...
  if (typeof req.session.data['individualStatement'] === 'undefined') {
    ...
    res.render('/v11/individual/psc-statement', {
      ...
      res.redirect('/v11/individual/psc-verified')
  }
})


// ******* extension reason javascript ********************************
router.get('/v11/extensions/extension-info', function (req, res) {
  // Set URl
  res.render('/v11/extensions/extension-info', {
    currentUrl: req.originalUrl
  })
})

router.post('/v11/extensions/extension-info', function (req, res) {
    res.redirect('/v11/extensions/extension-confirmation')
})
